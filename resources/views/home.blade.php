...
<section class="bg-stone-900/40 py-16 sm:py-24">
    <div class="mx-auto max-w-6xl px-4 sm:px-6">
        <div class="mb-10">
            <p class="text-sm font-semibold uppercase tracking-widest text-amber-500">{{ __('testimonials.section.eyebrow') }}</p>
            <h2 class="mt-2 text-3xl font-bold text-white sm:text-4xl">{{ __('testimonials.section.heading') }}</h2>
        </div>
        <div class="grid gap-6 sm:grid-cols-3">
            @foreach ($testimonials as $testimonial)
            <figure class="rounded-2xl border border-stone-800 bg-stone-950 p-6">
                <div class="flex gap-1" role="img" aria-label="{{ __('testimonials.section.stars_aria', ['rating' => $testimonial->rating]) }}">
                    @for ($i = 1; $i <= 5; $i++)
                    <svg class="h-4 w-4 {{ $i <= $testimonial->rating ? 'fill-amber-500' : 'fill-stone-700' }}" viewBox="0 0 20 20" aria-hidden="true"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 0 0 .95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 0 0-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 0 0-1.176 0l-3.976 2.888c-.783.57-1.838-.196-1.538-1.118l1.518-4.674a1 1 0 0 0-.363-1.118L2.077 10.1c-.783-.57-.38-1.81.588-1.81h4.914a1 1 0 0 0 .951-.69l1.519-4.674z"/></svg>
                    @endfor
                </div>
                <blockquote class="mt-4 text-sm leading-relaxed text-stone-300">&#8220;{{ $testimonial->text }}&#8221;</blockquote>
                <figcaption class="mt-4 text-sm font-semibold text-white">{{ $testimonial->name }}</figcaption>
            </figure>
            @endforeach
        </div>
    </div>
...
<section class="border-t border-stone-800 bg-stone-950/50">
    <div class="mx-auto flex max-w-6xl flex-wrap items-center justify-center gap-3 px-4 py-8 sm:px-6">
        ...
        <a href="{{ config('shop.gofood_url') }}" target="_blank" rel="noopener" class="rounded-full bg-red-500 px-6 py-2.5 text-sm font-semibold text-white transition hover:bg-red-400">{{ __('home.cards.delivery.gofood') }}</a>
        <a href="{{ config('shop.grab_url') }}" target="_blank" rel="noopener" class="rounded-full bg-green-600 px-6 py-2.5 text-sm font-semibold text-white transition hover:bg-green-500">{{ __('home.cards.delivery.grabfood') }}</a>
    </div>
</section>
