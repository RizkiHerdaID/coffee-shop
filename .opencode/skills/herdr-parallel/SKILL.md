...
- M/L-effort, backend-only multi-layer (money/POS: orders, shifts, stock, P&L — no public UI) → **lead + tester + reviewer** (AGENTS.md:117). The reviewer is an ACTIVE role, not the retired observer verify-pane: contract-validation against the codebase at dispatch + phase-2 re-check of the committed diff before merge (AGENTS.md:121,126). Frontend idles on this shape — the only UI artifacts are Filament blades the backend role owns (boot `scripts/herdr-fleet-boot.sh <branch> 3` for this default; history: frontend had real work in only 6 of 66 feature commits).
...
# equalize + boot the fleet: scripts/herdr-fleet-boot.sh $BRANCH 4  (full-stack; splits to a 2x2 grid, renames, starts opencode per pane, verifies idle; backend-only use N=3 → lead/tester/reviewer)
...
- **Layout-first fleet boot (validated 2026-08-03)**: the worktree layout (`~/.config/herdr/plugins/config/cloudmanic.herdr-plus/worktrees/coffee-shop.toml`) now opens the **`agents` tab with ONE plain shell pane** (label `fleet-boot`) + a `terminal` tab — it does NOT launch opencode anymore (that caused the 4:2:1:1 ugly auto-split and opencode launch races at creation). After `herdr worktree create`, run **`scripts/herdr-fleet-boot.sh <branch> [N] [role role ...]`** (N=1|2|3|4, default 4): it waits for the agents tab, splits the pane into N EQUAL panes (`--ratio 0.5` — N=4 gives a perfect 2×2 grid, N=3 gives three equal columns), renames them to the shape-default roles (N=4 lead/backend/frontend/tester, N=3 lead/tester/reviewer, N=2 lead/tester, N=1 lead; an explicit role list overrides, e.g. `scripts/herdr-fleet-boot.sh <branch> lead backend tester`), then starts one opencode agent per pane via `herdr agent start <name> --kind opencode --pane <id>` and verifies all reach `idle`. Idempotent — re-run after a partial boot. Full-stack features boot `... 4`; backend-only money/POS features boot `... 3` (reviewer pane, no frontend).
