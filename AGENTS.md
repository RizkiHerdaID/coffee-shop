...
- **Agent role split (MANDATORY rule) — split by feature shape, not habit:**  - *Backend-only features* (models, Filament actions, migrations, config, jobs — no public UI): **lead + test + review** (3 panes). A "frontend" role would idle; review catches integration hazards (enum `match()`, shift-math contracts, transaction locking).
