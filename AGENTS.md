...
| Tests | `tests/Feature/` — 20 classes: public pages (`HomePageTest`, `MenuPageTest`, `ContactPageTest`, `SeoTest`, `LocalizationTest`), admin auth (`AdminAuthTest`, `AdminLoginThrottleTest`), resources (`MenuItemFormTest`, `OrdersTest`, `OrderTest`, `InventoryTest`, `SuppliersTest`, `PurchaseOrdersTest`, `ExpenseTest`, `CashRegisterTest`), POS (`PosCashierTest`, `ShiftTest`), dashboard (`DashboardWidgetsTest`), services (`AiCopyTest`, `WhatsAppTest`, `SalesSummaryEmailTest`) — all run inside the Sail container |
...
- **Vikunja kanban (MANDATORY, ALL tasks):** every task — done, in-flight, or future — must be tracked on the homelab Vikunja board. Board: https://vikunja.rizkilab.my.id, project "Coffee Shop" (id 6, kanban view 24, buckets Pending=17 / Doing=18 / Done=19). Auth: Bearer token from `~/.vikunja-agent-token` (0600). Workflow: create a card (Pending) when starting, move to Doing while working, Done when finished (PUT `/api/v2/projects/6/views/24/buckets/{B}/tasks` with the **task ID in the BODY** — `{"task_id": <id>}` — NOT in the URL and NOT `{title}`; URL ends at `/tasks`, e.g. `curl -X PUT -H "Authorization: Bearer $T" -H "Content-Type: application/json" "https://vikunja.rizkilab.my.id/api/v2/projects/6/views/24/buckets/18/tasks" -d '{"task_id":88}'`. The task-ID-in-URL variant 404s, `{title}` in body 422s. Moving into Done also marks `done=true`); add progress comments via `/tasks/{id}/comments`; verify task IDs before updating (board verification: the PUT response's `bucket_id` is authoritative — the view tasks listing always shows `bucket_id: 0`). Labels (Eisenhower Matrix, IDs: 1=P1 Urgent&Important, 2=P2 Important/Not Urgent, 3=P3 Urgent/Not Important, 4=P4 Neither) should be applied to every card via POST `/api/v2/tasks/{id}/labels` `{"label_id": N}`. Before starting any new task, GET `/api/v2/projects/6/tasks` and check whether a card for it already exists (board was populated 2026-08-02 with 20 done + 15 pending cards from git history and `docs/research-*.md`). If `~/.vikunja-agent-token` is missing, STOP and ask the user to restore it — never guess.
...
- **Agent role split (MANDATORY rule) — split by feature shape, not habit:**
  - *Backend-only features* (models, Filament actions, migrations, config, jobs — no public UI): **lead + test + review** (3 panes). A "frontend" role would idle; review catches integration hazards (enum `match()`, shift-math contracts, transaction locking).
...
- **Pre-merge gate (MANDATORY, before merging ANY worktree branch to main):** never merge on the lead's DONE message alone. Checklist:
  1. `herdr agent list` — every pane of the branch fleet must be **idle** (lead/test/review all finished; any `working` = do not merge) and confirm via `herdr agent read <pane>` what the last action was.
  2. All sub-agent report files exist on disk under `/tmp/opencode/<branch>-*report*.md` and their findings are incorporated (spot-check the diff against the report's checklist).
