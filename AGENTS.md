...
2026-08-04 audit-iteration marathon (all on `main`, from the wave-4 audit + learning fleets):
`6bcc6db` wastage edit/delete re-adjusts stock + ExportsTest parallel flake fix → `a0f3edb` missing model factories added → `8dd79e0` JSON-LD blocks escaped against script-tag breakout (stored XSS) → `1b9d410` money-freeze guards (closed shifts frozen against post-close payment/served mutations, deadlock-safe) → `f9deed6`+`57d2049` summary email apportions order-level discounts across line items (percent-discount regression pinned) → `bf5e19b` Fonnte timeouts + worker retry window, WA job dispatch-time locale, reservation job null-guard → `f3ced83` audit cascades (admin FKs nullOnDelete, supplier/stock-item FKs restrict, delete guards, received-PO freeze) → `4ab335a` public site (/cek-poin throttle, reservation hours/future-cap/phone validation, locale round-trips) → `47d2489` test hygiene (de-flake order numbers + sitemap lastmod, factories wired to models) → `2e37885` resource hardening (stockIn-0 guard, form validations, redeem notification, Carbon locale).
...
| Seeders | `database/seeders/DatabaseSeeder.php` (User factory + Admin from `ADMIN_NAME`/`ADMIN_EMAIL`/`ADMIN_PASSWORD` env with fallbacks `Admin`/`admin@example.com`/`password` + `MenuSeeder` + `StockSeeder` + `PromoSeeder`); `MenuSeeder.php` — 8 items with Indonesian notes, idempotent via `updateOrCreate` on `name` (re-seed `--class=MenuSeeder` to refresh notes; `menu_items`/`stock_items` have UNIQUE `name` and `promos` UNIQUE `title` since 2026-08-03); `StockSeeder.php` (4 items) + `PromoSeeder.php` (1 always-live promo) — idempotent via `firstOrCreate` on `name`/`title` since 2026-08-04, refresh only static copy fields and never clobber live stock quantities/dates |
| Tests | `tests/Feature/` — 57 classes, **689 tests / 3057 assertions** (verified 2026-08-04): public pages (`HomePageTest`, `MenuPageTest`, `ContactPageTest`, `SeoTest`, `LocalizationTest`, `QrMenuTest`, `ReservationTest`, `SecurityHeadersTest`, `PageSpeedTest`, `PointsPageTest`, `RobotsTest`), admin auth (`AdminAuthTest`, `AdminLoginThrottleTest`), resources (`MenuItemFormTest`, `OrdersTest`, `OrderTest`, `InventoryTest`, `SuppliersTest`, `PurchaseOrdersTest`, `ExpenseTest`, `CashRegisterTest`, `PromoTest`, `RecipesTest`, `TestimonialsTest`, `WastageTest`), POS (`PosCashierTest`, `ShiftTest`, `PaymentTest`, `RefundsVoidsTest`, `RecipeStockConsumeTest`, `OverTenderRegressionTest`, `PaymentChangeTrackingTest`, `PosConcurrencyTest`, `OrderRefundGuardTest`, `MoneyFreezeDeadlockTest`), dashboard (`DashboardWidgetsTest`, `LowStockWidgetTest`, `DemandForecastTest`, `PnlReportTest`), services (`AiCopyTest`, `WhatsAppTest`, `SalesSummaryEmailTest`, `WaPickupTest`, `LowStockAlertTest`, `LoyaltyTest`, `PhoneTest`, `SendOrderConfirmationTest`, `SendReservationConfirmationTest`), ops (`ExportsTest`, `PulseTest`, `UptimeMonitorTest`, `ScheduleRegistrationTest`, `MenuSeederTest`, `StockSeederTest`, `PromoSeederTest`, `FactorySmokeTest`, `ForeignKeyCascadeTest`) — all run inside the Sail container |
