...
6. **Worktree Sail containers conflict on ports.** Feature branches run in herdr worktrees (`~/.herdr/worktrees/coffee-shop/<branch>`); their `sail up` containers (e.g. `feature-menu-redis-1`) hold ports 6379 etc. Before `sail up` in the main repo, check `docker ps -a` and `sail down` in stale worktrees.
